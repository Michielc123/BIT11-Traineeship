...
#' DISCLAIMER: due to the nature of bendability problem and limitations of extended
#' genetic alphabet, output of this function is not intended for further machine use.
#' Not all sequences which can be inferred from consensus output will have satisfied
...
#'    input sequences will be chosen at random. Can also be a vector of desired sequences'
#'    indexes in the input.
#'
#' @return A data.table containing variable number of consensus sequences written in
#'    IUPAC's extended genetic alphabet.
...
#'
#' For the extended genetic alphabet, see \code{\link[Biostrings]{IUPAC_CODE_MAP}}.
#' @export
#'
#' @examples
#' dt <- MatchBendability("TGATTCCTAAAGTCA", "con", k=3, tolerance=0.1)
#' ClusterAndConsensus(dt$sequence, cutoff=0.25)
#' ClusterAndConsensus(dt$sequence, cutoff=0.85)
