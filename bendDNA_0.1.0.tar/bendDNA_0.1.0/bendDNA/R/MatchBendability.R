...
MatchBendability <- function(query, scale, k, tolerance, wsize=NULL, output.list=F, random.out=F, size.cutoff=20, frac=0.2) {

  # get bendability scale and k-mer lookup table:
  bendtable <- TrinucleotideScales(scale)
  lookup <- LookupTable(scale, k)
  ...
  PickRandomly <- function(dt, size.cutoff, frac) {
    if(nrow(dt)>size.cutoff) dt <- dt[, .SD[sample(.N, max(1, round(.N*frac)))], by=.(pref, suff)][, c(3:5, 1:2)]
    return(dt)
  }
  ...
  if(is.null(wsize)){
    ...
    # are required, the list has to be iteratively merged, which will greatly increase the runtime:
    if(output.list==T) { out <- lapply(filtered, function(x) x[, c(1:3)])
    } else {
      out <- filtered[[1]]
      for (i in 2:length(l)) {
        out <- out[filtered[[i]], on=.(suff=pref), allow.cartesian=T, nomatch=0
                   ][, ':=' (sequence=paste0(sequence, stringr::str_sub(i.sequence,3,wsize)), deltabend=(deltabend+i.deltabend), strdist=(strdist+i.strdist), pref=pref, suff=i.suff)
                     ][, .(sequence, deltabend, strdist, pref, suff)]
      }
      out <- out[, .(sequence, deltabend, strdist)]
    }
  }
  return(out)
}
